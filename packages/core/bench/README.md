...
| `corpus/bench-argv.diff`       | byte-exact output of `git show --format= 6135228` in this repository — a real diff, committed as bytes because a diff is not a file the repository holds. A **content-kind probe**: a piped diff is path-less, detects `unknown`, and can only ever reach the lexical planner; the row measures whether that planner fits the budget on it                                                                                                                                                                                                                                                                                 |
| `corpus/judge-log`             | materialized at run time from this repo's `packages/core/bench/ab-log/java-classes.json` (a real tier-4 A/B log), sha256-pinned by the committed `corpus/judge-log.json` reference. The second content-kind probe: JSON under the lexical planner, which sees lines and not structure. No `.json` suffix on the materialized name, because every `corpus/*.json` is read as a reference                                                                                                                                                                                                                                    |
