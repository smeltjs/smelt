...
| `packages/core/src/hooks/invocation.ts`  | How smelt is re-invoked on this machine, answered once: `isSameFile` (realpath both sides, so a shim reached through a symlink is still the main module), `stableScriptPath` (a versioned Homebrew keg path rewritten to the `opt` alias `brew upgrade` re-points), `smeltOnPath`, and `smeltInvocation()` — the ranked value every writer asks for instead of deriving a command of its own. Node builtins only, like its sibling guard core, which reads it. |
