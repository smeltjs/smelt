...
| `packages/core/src/cli/lava.ts`             | The **Palette**: every byte of colour smelt writes, and the primitives under it (`kv`, `table`, `bar`, `glyph`, `percent`, `logo`). Roles, never colours, at a call site; off is the identity, padding is measured before painting, and a non-zero never rounds to zero.                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| `packages/core/src/cli/setup.ts`            | `smelt setup` — the SetupRecipe applied end-to-end: config, the hooks preset over `planInstall` (which also carries the MCP registration, JSON or TOML, for every profile that declares one), the manual MCP handover, in the words that harness's own docs use (`HarnessProfile.mcp`), with `mcp.commands` naming every registration a run is about, and a real smelt → retrieve round trip. One apply path under `--yes` and the wizard, and the merge policy is `cli/merge-policy.ts`'s, applied by policy consent: an existing file is merged, a whole-owned file that is not smelt's is refused. `smelt.config.json` is written once per run. A re-run is a byte-neutral no-op. |
