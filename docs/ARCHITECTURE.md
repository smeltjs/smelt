...
(`OPT_IN_RERANK_PACKAGES`), `rerank/resolve.ts` turns that value into a `file:` URL and
`rerank/load.ts` hands _that_ to `import()` when a consumer's own `rerank` config block
asks for it, and three mutations prove the distinction is real: a static import of the
adapter in either package goes red, and so does respelling the loader's dynamic import
with a string literal. That last one is the important one — it changes nothing about
the running code, and everything about whether the adapter is in the graph.
...
hash. The stage's shape is written down under "Explicitly out of scope" below, as prose
— there is no interface for it, on purpose (ADR-0005).
...
- **Reversible until the user prunes — and the prune is itself counted.** One global
  ...
  The cut-off may be **written down** — `store.retention.olderThan` in
  `smelt.config.json`, which `--older-than` overrides and the receipt attributes — and
  that is not a hole in the ruling: the doctrine is about the deletion, not the number.
  A key in the user's own file schedules nothing, is read by one verb at the moment that
  user types it, and with neither spelling present the verb still refuses.
...
| File                                        | What it does                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| ------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `packages/core/src/types.ts`                | The vocabulary: `Planner`, `ElisionPlan`, `AppliedElision`, `ElisionStore`, `RetrieveStats`, `Measure`, `RerankStage`, `RerankAttribution`. Read this first — the doc comments carry the reasoning.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
| `packages/core/src/store-dir.ts`            | `DirectoryElisionStore`: the persistent store — one file per content hash, atomic no-clobber writes, verify-on-read, counters in an append-only journal. `survey()` answers the counters, the ledger and the size from one blob scan plus one journal fold, and `stats()`/`rawCounters()` are views over it; `ledger()` takes the journal half alone, because `smelter.ts` asks for it on every run and no fact in it lives in `blobs/`. See "The persistent store" below.                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `packages/core/src/store-cutoff.ts`         | The age cut-off grammar (`<n>d`/`<n>h`/`<n>w`) and its one reader — shared by `--older-than` and `store.retention.olderThan`, so a flag and a config key cannot disagree about what `30d` is worth. Reads a spelling; words no refusal, because the two callers do not refuse alike.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| `packages/core/src/apply.ts`                | `applyPlan()` (the only function that removes anything), `reconstruct()` (Law 3 as an equation), `MARKER_FORMAT_VERSION` — the wire surface, frozen — and `markerScheme()`, which mints the marker builder and the pricing that measures exactly its markers as one value, so a cut's cost and its bytes cannot disagree (review IV, REP-53). The one judgement here is which marker a language gets (`markerForLanguage`); everything else is mechanism.                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
| `packages/core/src/plan/focus.ts`           | The Focus, matched — `focusMatcher` over the caller's terms and the one `FocusOptions` (`caseSensitive`) every planner option bag extends: substring, case-folded unless asked otherwise, empty terms dropped, the first matching term in caller order reported for receipts. The four planners and the repo map ask it; none carries a copy (review IV, REP-52).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| `packages/core/src/cli/wizard.ts`           | The wizard kit — the ask adapter, the step machine with real back, the confirms, the plan listing (padded to its own longest name), the file-fate label under a consent policy, the one per-file overwrite question, and the one write mechanic — shared by `init`, `hooks`, `setup` and `agents split`. Extracted (review II) because the third copy of the stream machinery was the one that raced; finished (review IV, REP-55) because three copies of the consent question sat outside it, and `test/guards/module-seams.test.ts` now pins each of those symbols to this file.                                                                                                                                                                                                                                                                                                                                            |
...
| `packages/core/src/harness/hook-command.ts` | What one hook entry says, as a value, and both directions over it: `renderHookCommand` is the only writer, `parseHookCommand` the only reader (three quotings, the `$(readlink -f …)` workaround, `undefined` for a foreign entry). A pure round trip — no process, filesystem or clock in it, and the module-seams guard pins that — because its callers (the installed-state reader, the install planner) only parse.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| `packages/core/src/harness/hook-probe.ts`   | The probe, split from the parser (review IV, REP-57) and imported only by `smelt doctor`: `probeHookCommand` runs a parsed command against a payload built from the harness's own hook schema, in a scratch directory with the threshold pinned, under a timeout, so doctor can say `wired (verified)` rather than `wired`. `probeOwnFile` is the same answer for a file smelt owns whole (Cline, Hermes, opencode), folding over the `HarnessOwnFileProbe` its profile declares: the command behind the renderer's own prefix, or an ES module loaded to prove its import graph resolves and it still exports the hook. The one spawn Law 1 permits lives here, and runs this very node.                                                                                                                                                                                                                                      |
...
| `packages/core/src/index.ts`                | The public surface, as a barrel — and only what ADR-0005 admits: documented, consumed by a workspace package or generator, reachable from a public signature, or listed with a reason in `test/guards/public-surface.test.ts`. `createSmelter()` itself lives in `src/smelter.ts`, so nothing under `src/` imports the barrel to build a smelter.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |

### Stubs that throw (by design — read `CONTRIBUTING.md` § "A stub throws")

| File                          | Why it throws                                                                                         |
| ----------------------------- | ----------------------------------------------------------------------------------------------------- |
| `packages/core/src/stages.ts` | `unconfiguredRerankStage`. It names the interface you were meant to implement. Out of v1 — see below. |
...
| `packages/core/test/guards/public-surface.test.ts`      | The barrel, held to ADR-0005 in both directions. Every export of `src/index.ts` is documented (a backticked name in the README, ARCHITECTURE, SETUP or a package README), consumed (imported from `@smeltjs/core` by a workspace package, or read off the barrel by a repository generator), reachable (a type in the signature of something public, walked over the emitted `.d.ts` files, since TypeScript 7 ships no programmatic checker), or listed under a one-line reason — and every name a consumer imports is still exported. A stray internal on the barrel is red; so is un-exporting what `@smeltjs/mcp` builds against.                                                                             |
...
| `packages/core/test/guards/skill-pack.test.ts`          | The published SkillPack teaches only what the recipe and the MCP server say, and every file of it is the generator's output byte for byte: a hand edit to the root or a reference, or a stray file, is a red verify; the root is held under its stated byte budget by a recount the guard makes itself; every reference is reachable from the root; the MCP tool names are pinned to the server's own source; every command the recipe carries reaches the pack; the description opens with its trigger; and Law 4 holds the prose.                                                                                                                                                                               |
...
| `scripts/generate-skill.mjs`           | Renders the published SkillPack — the root `SKILL.md` router and its `references/*.md` — from the built SetupRecipe, so the teaching channel for agents that never ran the installer cannot drift from the commands the installer runs; refuses a root over `ROOT_BUDGET_BYTES`.                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
**Two modules read a hook entry, and only one runs it.** `harness/hook-command.ts` is
the value and its round trip — what an entry _says_, rendered and parsed, with no
process, filesystem or clock in it, because its callers (the installed-state reader,
the install planner) only parse. `harness/hook-probe.ts` is the prober `smelt doctor`
runs to say `wired (verified)` rather than `wired`: a real spawn of this node against a
synthetic payload in a scratch directory, under a timeout. The module-seams guard pins
the parser's imports to the value-only set and doctor as the probe's only importer
(review IV, REP-57).
...
terms every other elision uses, and is reversible — or it does not ship. That shape is
written down here, as prose, and nowhere as an interface: an exported `DistillStage`
used to exist for the purpose, and an interface whose only adapter throws is an
invitation to fill it in quietly, so ADR-0005 deleted it.
