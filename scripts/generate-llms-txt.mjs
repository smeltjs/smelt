...
function documents(adrs) {
  return [
    ...
    },
    {
      ...
      note: 'The instructions an agent runs on, installed by its owner: the root router — read, retrieve, map, obey a guard denial, the MCP names. Generated from the SetupRecipe, never hand-written.',
      inline: true,
    },
    {
      section: 'Docs',
      path: 'skills/smelt/references/setup.md',
      title: 'SkillPack — setup and doctor (skills/smelt/references/setup.md)',
      note: 'The operator half the root routes to: install, `smelt setup`, `smelt doctor`, the upgrade loop, and the step-by-step fallback for older installs.',
      inline: true,
    },
    {
      section: 'Docs',
      path: 'skills/smelt/references/store.md',
      title: 'SkillPack — the store (skills/smelt/references/store.md)',
      note: '`smelt store prune`, the cut-off, and the `store.retention` block. Read only when keeping a store small is the job.',
      inline: true,
    },
    {
      section: 'Docs',
      path: 'skills/smelt/references/rerank.md',
      title: 'SkillPack — the reranker (skills/smelt/references/rerank.md)',
      note: 'The opt-in rerank stage, where its adapter is looked for, and what `topK` means. Read only when configuring one.',
      inline: true,
    },
    {
      section: 'Docs',
